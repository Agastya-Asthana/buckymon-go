...
app = Flask(__name__)
# CORS(app, resources={r"/api/*": {"origins": "http://localhost:3000"}}, origins=['http://localhost:3000'])
...
def Get_List_Of_Locations():
    ...
    for dictionary in result:
        ...
        dictionary['lat'] = round(float(dictionary.pop("l_lat") ), ndigits=8)
        dictionary['long'] = round(float(dictionary.pop("l_long")), ndigits=8)
