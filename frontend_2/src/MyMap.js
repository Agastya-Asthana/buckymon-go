...
function MyMap(props){
    ...
    const [user_score, Set_User_Score] = React.useState(0)
    ...
    const Fetch_Locations = async () =>{
        ...
        const res = await fetch(`http://localhost:5000/api/Get_List_Of_Locations`, {
            method: "GET"
        })
    ...
    const Fetch_CompletedEvents = async () =>{
        ...
        let query = Object.keys(params)
                          ...
                          .join('&')
        let url = `http://localhost:5000/api/Get_Completed_Events?` + query
    ...
    const Fetch_Active_Events = async () =>{
        ...
        let query = Object.keys(params)
                          .map(k => encodeURIComponent(k) + '=' + encodeURIComponent(params[k]))
                          .join('&')
        let url = `http://localhost:5000/api/Active_Events?` + query
        const res = await fetch(url, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
    ...
    const Fetch_Achievements = async () =>{
        ...
        let query = Object.keys(params)
                          .map(k => encodeURIComponent(k) + '=' + encodeURIComponent(params[k]))
                          .join('&')
        let url = `http://localhost:5000/api/Get_User_Achievements?` + query
        const res = await fetch(url, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
                ...
                {/* for each location, place a marker on the map */}
                {
                    markers ? 
                            markers.map(marker => {
                                let status = ""
                                let event = null
                                if(event = active_events.find(event => event.event_id === marker.event_id)){
                                    status = "active"
                                }
                                else if(event = completed_events.find(event => event.event_id === marker.event_id)){
                                    status = "completed"
                                }
                                return(
                                    <MyMarker 
