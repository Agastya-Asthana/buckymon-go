...
function MyMarker(props){
    ...
    const Handle_Complete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/Complete_Event`, {
                method: 'POST',
                headers: {
        ...
        color = "red"
        props.Set_User_Score(props.event.event_score);
    };
                ...
                >
                        <h3>{props.location_name}</h3>
                        {
                            props.event ?
                                ...
                                : 
                                    <div>
                                        <p>{`Congratulations on completing ${props.event.event_name} for ${props.event.event_score} points!`}</p>
                                    </div>
                            : <></>
                        }
